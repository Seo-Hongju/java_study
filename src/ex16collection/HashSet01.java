...
HashSet : Set계열의 인터페이스를 구현한 컬렉션으로
	객체가 순서없이 저장된다
	객체의 중복저장이 허용되지 않는다. 단, 기본클래스가 아닌
		새롭게 정의한 클래스라면 hashCode, equals메소드를
		적절히 오버라이딩해야 한다. 
...
public class HashSet01 {
	...
	public static void main(String[] args) {
		...
		/*
		1]set컬렉션 생성
			: 생성시 제네릭 타입을 지정하지 않건, Object로
			지정하면 모든 객체를 저장할수 있는 컬렉션이 된다. 
		 */
		HashSet<Object> set = new HashSet<Object>();

		//2] 다양한 객체 생성
		String strObject1 = "JAVA";
		String strObject2 = new String("KOSMO51기");
		 ...
		 */
		Teacher teacher2 = 
				new Teacher("김봉두", 55, "체육");
		System.out.println(set.add(teacher2) 
				? "성공" : "실패");/*
					동일한 정보를 가진 객체이지만 사용자가
					정의한 클래스의 객체이므로 중복저장됨.
					중복을 확인하기 위해서 hashCode, equals
					메소드의 오버라이딩이 필요함.
				*/
		...
		while(itr.hasNext()) {
			...
			/*
			저장된 모든객체는 Obejct타입으로 형변환되어
			저장되므로 출력할때는 어떤타입의 객체인지를 
			instanceof 연산자를 통해 확인해야 한다. 
			 */
