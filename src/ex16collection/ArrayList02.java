...
public class ArrayList02 {
	...
	public static void main(String[] args) {
		...
		//List계열의 컬럭션 생성. Student객체를 저장할수있음.
		//ArrayList<Student> list2 = new ArrayList<Student>();
		/*
		ArrayList와 LinkedList의 멤버메소드가 완전히 동일하므로
		별도의 변경이 필요없다. 단지 내부적으로 데이터를 처리하는
		방식(자료구조)만 다르다. 데이터의 참조는 ArrayList가 훨씬
		빠르므로 일반적으로 많이 사용된다. 
		 */
		...
		/*
		객체삭제 : 동일한 객체가 2개이상 저장된 경우에는 인덱스가
			빠른 하나만 삭제된다. 
		 */
		list2.remove(st2);
...
//			student.showInfo();
		...
		for(int i=0 ; i<list2.size() ; i++) {
			/*
			List계열의 컬렉션은 인덱스를 통해 접근하는것이
			가능함. 이때 get()메소드를 사용한다. 
			 */
		...
		/*
		remove()를 통해 객체를 삭제하면 삭제된 객체의 참조값을
		반환한다. 
		 */
		System.out.println("삭제된 객체의 이름:"+ 
				list2.remove(2).getName());//공유 객체 삭제
