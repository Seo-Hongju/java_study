...
HashMap<K,V>
	...
	Key, Value의 쌍으로 객체를 구성한다.
	키값은 중복될수 없다. 만약 중복될경우 덮어쓰기가 된다.
	즉 기존값이 삭제된다. 
...
public class HashMapMain {
	...
	public static void main(String[] args) {
		...
		2.객체저장
			: String put(키, 밸류);
			여기서 반환되는 값은 String 타입으로 해당 키값으로
			저장된 이전객체의 value값이 반환된다. 만약 이전에
			저장된 객체가 없다면 null이 반환된다. 
		 */
		System.out.println("name이라는 키값으로 저장된 "
			+ "이전의 값:"+
		...
		*/
		...
		/*
		5-2]키값을 모를때 출력하기
			Set<T> KeySet() 메소드 호출을 통해 키값을 Set
			계열의 컬렉션형태로 반환한다. 
			Map계열의 컬렉션은 처음부터 확장for문을 사용하는
			것이 불가능하고, 키값을 얻어온후 사용해야한다. 
		 */
		...
		/*
		5-3]반복자를 통한 출력 
			: Iterator를 사용할때도 keySet()으로 키값을 먼저
			얻어온후 출력할수 있다. 
		 */
		...
		/*
		6]존재유무확인 : 해당 key 혹은 value의 존재유무만 
			확인할수 있다. 존재할 경우 true 반환함.
		 */
		...
		/*
		7]삭제 : 삭제가 완료되면 삭제된 key값에 해당하는 value가
			반환된다. 
		 */
		System.out.println("삭제된객체:"+ 
				map.remove("age"));
