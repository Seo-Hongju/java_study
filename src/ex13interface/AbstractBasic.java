...
class NoHavingChild extends NoHavingAbsMethod {
	...
	오버라이딩은 강제사항이 아니므로 필요에 따라 처리하면 된다.
	 */
	@Override
	void instanceMethod() {
		//강제사항아님
...
class HavingChild extends HavingAbsMethod {
	/*
	의무적으로 오버라이딩한 메소드
	이경우 오버라이딩 하지 않으려면 해당 클래스를 abstract로
	정의해야한다. 그럴경우 해당 클래스로는 객체를 생성할수 없게
	되므로 오버라이딩은 필수적이다. 
	 */
	@Override
...
public class AbstractBasic {
	...
	public static void main(String[] args) {
		...
		/*
		추상클래스로 객체생성은 할수 없지만, 참조변수로 사용
		하는것은 가능하다. 부모객체로 자식객체를 참조하게되면
		관리의 일관성을 유지할 수 있다. 
		 */
		HavingAbsMethod ham = new HavingChild();
		ham.absMethod(100);
		/*
		부모객체로 자식객체를 참조하게되면 업캐스팅(자동형변환)
		이 되므로 다운캐스팅(명시적형변환)을 통해 자식객체에
		접근해야 한다. 
		 */
		((HavingChild)ham).newMethod();
		
		/*
		추상클래스에 선언된 정적멤버는 오버라이딩의 대상이
		될수 없으므로 static이 가진 기본규칙을 따른다. 클래스명
		으로 접근이 가능하다. 
		 */
