...
/*
추상메소드를 포함한 추상클래스 정의. 해당 프로그램에서는 
설계도의 역할을 한다. (교안 시나리오 참조)
 */
...
위 추상클래스를 인터페이스로 변경하려면
 	abstract class => interface 로 변경
 	메소드의 경우
 		public abstract를 삭제
 	변수의 경우
 		public static final을 삭제 하면 된다. 
 */
...
/*
클래스가 인터페이스를 상속할때는 implements를 사용하고, 
	"구현"이라고 표현한다. 
 */
	...
	public void addPersonalInfo(String juminNum, String name) {		
		/*
		이름과 주민번호를 전달받아 객체생성후 객체배열에 정보를
		저장한다. 
		 */
		personalArr[numOfPerInfo] = 
				new PersonalNumInfo(name, juminNum);
		numOfPerInfo++;		
	}	
	@Override
	public String searchPersonalInfo(String juminNum) {
		/*
		주민등록번호를 매개변수로 받아서 정보를 검색한후 일치하는
		경우 이름을 반환한다. 루프는 현재 입력된 정보의 갯수만큼
		반복한다. 
		 */
