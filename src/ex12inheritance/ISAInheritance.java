...
IS-A 관계
	...
	노트북은 컴퓨터의 일종이다
	와 같이 IS-A관계가 성립하는것을 상속으로 표현하기에 적합하다. 
 */
...
class Computer{
	...
	/*
	private접근지정자로 선언된 멤버메소드는 외부클래스 혹은
	상속받은 클래스에서 접근이 불가능하므로 public으로 선언된
	멤버메소드를 통해 간접적으로 호출한다. 상속관계이므로
	protected로 선언된 메소드로 호출할수도 있다. 
	 */
}
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/*
		부모클래스의 생성자호출을 위한 super()는 생략은 
		가능하지만, 다른 문장이 먼저 기술되면 오류가 발생되므로
		주의해야 한다. 
		 */
		super(name);
		battary = initCharge;
