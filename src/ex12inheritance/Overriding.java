...
오버라이딩(Overriding)
	...
	멤버메소드를 동일한 이름으로 메소드를 재정의하는것을 말한다.
	  오버로딩(Overloading)과 다른점이라면 매개변수의 갯수, 
	타입,반환형이 완전히 동일한 형태로 정의된다. 
	  하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를
	가리게된다. 즉 하위클래스에서 재정의한 메소드가 호출되게
	된다.
 */
class Speaker{
...
public class Overriding {
	public static void main(String[] args) {
		...
		접근범위는 Speaker 객체로 제한된다. 따라서 하위클래스에
		정의된 setBase()메소드는 호출할수 없다. 
		단, 오버라이딩 된 메소드가 있다면 하위클래스의 메소드가
		호출되게 된다. 		
		 */
		speaker.showState();
	}
