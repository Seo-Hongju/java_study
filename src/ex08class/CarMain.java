...
class Car {
	...
	/*
	멤버상수 선언
	※static으로 선언된 변수(or 상수)는 메소드영역에 미리
	생성되어 프로그램내에서 언제든 접근할수 있다. 
	 */
	...
	int carYear;
	Human owner;/*
		소유주를 표현 : Human클래스를 기반으로 하는 
			멤버변수로 이름, 나이, 에너지 세가지 값을
			필요로 한다. 
	*/
	...
	void drive() {
		/*
		Car클래스의 멤버변수로 Human객체가 선언되었으므로
		출력할때는 .(닷)연산자를 이용해서 name변수에
		접근한다. 
		 */
	...
	/*
	객체의 초기화를 담당하는 메소드로 아래는 항상 동일한
	값으로만 초기화 된다는 단점을 가지고있다. 
	 */
	void initialize() {
		carModel = "람보르기니";
		carYear = 2017;
	...
	/*
	위와 동일한 이름으로 오버로딩에 의해 정의된 메소드로
	다양한 매개변수를 통해 초기화 하는것이 가능하다. 
	 */
	void initialize(String model, int year, 
		String name, int age, int energy) {
		carModel = model;
		carYear = year;
		owner = new Human();
		owner.name = name;
