...
1.쿼리문을 준비한다. 
	...
	1-2]인파라메타가 있는 쿼리문
		...
		?(물음표)로 대체 작성한후 setXXX() 계열의 메소드를 
		통해서 값을 입력한다. 
		"insert into 테이블 values (?,?)";
		
2.쿼리실행을 위한 PreparedStatement 객체를 생성함
	객체생성시 미리 쿼리를 준비해서 Connection객체의 메소드를 
	통해 인자로 전달한다. 이때 전달된 쿼리문은 먼저 
	parsing(파싱)되고 차후 값을 입력받는다.
...
public class InsertSQL extends IConnectImpl {
	...
	public void execute() {
		try {
			...
			//4.인파라미터 설정하기 : ?의 인덱스 순서대로 설정함.
			psmt.setString(1, id);
			psmt.setString(2, pw);
			psmt.setString(3, name);
			
			/*
			날짜를 문자열 형태로 입력하는 경우에는 JAVA에서
			생성한후 그대로 입력한다. 
			 */			
			//psmt.setString(4, "2018-11-20");
			
			/*
			현재날짜를 JAVA에서 입력하는 경우 먼저 util패키지의
			Date객체로 현재날짜를 가져온후 sql패키지의 Date객체로
			변환후 오라클에 입력해야 한다. 
			*/
