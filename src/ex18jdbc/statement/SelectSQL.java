...
*/
			...
			while(rs.next()) {
				...
				/*
				date형 자료를 getString() 메소드로 받아와서 출력하면
				2019-11-30 15:23:04 형태로 출력된다. 
				*/
				/*String regidate = rs.getString(4);*/
				
				/*
				date형 자료를 getDate() 메소드로 받아와서 출력하면
				2019-11-30 형태로 출력된다. 단 반환타입은 java.sql.Date
				형이므로 주의해야 한다.
				 */
...
/*
ResultSet 클래스
	: select문 실행시 쿼리의 실행결과가 ResultSet객체에 
	저장된다. 결과로 반환된 레코드의 처음부터 마지막까지 
	next()메소드를 통해 확인후 반복하면서 추출하게 된다.
	...
	-getXXX()계열의 메소드
		...
		컬럼명, 별칭을 사용할수 있다. 
		단, 자료형에 상관없이 모두 getString()으로 추출할 수 
		있다. 
*/
	public static void main(String[] args) {
		SelectSQL selectSQL
			= new SelectSQL(ORACLE_URL, "korea", "1234");
		selectSQL.execute();
	}
}