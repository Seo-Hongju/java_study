...
class MyClass {	
	...
	/*
	toString()
		: 실제 저장된 데이터가 반환되도록 Object클래스로부터
		상속받아 오버라이딩한 메소드.
	 */
	@Override
	...
	/*
	hashCode()
		: 객체의 고유한 주소값을 문자열의 형태로 반환한다. 차후
		set컬렉션에서 중복제거를 위해 사용될 예정임. 
	 */
	@Override
	public int hashCode() {
		return data;
	...
	/*
	equals()
		: 두 객체간의 내용비교를 위해 오버라이딩한 메소드로
		멤버변수를 비교해서 동일여부를 판단한다. 
	 */
	@Override
	public boolean equals(Object obj) {
		/*
		비교의 대상이 되는 객체를 매개변수로 전달받아 해당
		객체타입인지 확인후 내용을 비교한다. 
		 */
		if(obj instanceof MyClass) {	
			/*
			매개변수로 전달될때 Object로 업케스팅(자동형변환)
			되기때문에 실제 값을 비교하기 위해서 다운케스팅을
			진행한다. 
			 */
...
class Point {
	...
	/*
	객체1의 x,y값과 객체2의 x,y값을 각각 비교하여 동일한 객체
	인지 판단한다. 
	따라서 객체의 형태에 따라 equals()메소드의 내용은 달라질수
	있다. 
	 */
	@Override
	public boolean equals(Object obj) {
...
public class ObjectEquals2 {
	...
	public static void main(String[] args) {
		...
		/*
		아래 toString()메소드는 굳이 호출하지 않아도 print()
		문이 실행될때 자동으로 호출된다. 즉 아래문장의 실행
		결과는 동일하다. 
		 */
