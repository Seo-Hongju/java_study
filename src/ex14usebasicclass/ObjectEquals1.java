...
/*
equals()메소드
-자바에서 인스턴스의 내용비교를 위한 비교연산자(==)을 사용하는
것은 단순한 참조값에 대한 비교이므로 실제 내용에 대한 비교는 
equals()메소드를 사용한다. 
-String과 같은 기본클래스는 별도의 오버라이딩 없이도 내용비교가
가능하다.
-개발자가 직접 정의한 클래스인 경우 내용 비교를 위해서는 반드시
오버라이딩 하여 사용해야 한다. 
 */
class IntNumber {
	int num;
	...
	/*
	아래 메소드는 equals()메소드가 어떻게 객체간의 내용비교를
	하는지 설명하기 위한 샘플코드이다. 
	 */
