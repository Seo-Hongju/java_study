...
public class WrapperClass2 {
	...
	public static void main(String[] args) {
		...
		*/		
		...
		//System.out.println(
		//	Integer.parseInt(floatString));[오류발생]
		...
		2] static int codePointAt(문자열, 인덱스)
			: 문자열에서 특정 index에 해당하는 문자의 아스키코드
			값을 반환해준다. index는 0부터 시작이다. 
		 */
		System.out.println("ABCD에서 3번째 인덱스 D의 아스키코드");
		System.out.println(
				Character.codePointAt("ABCD", 3));//결과:68
	
		/*
		3] static boolean isDigit(문자)
			: 인자로 전달된 문자가 숫자인지를 판단하는 메소드.
			단 주어진 문자가 숫자 혹은 숫자형문자일때만 숫자로
			판단한다. 
		 */
		System.out.println("isDigit()를 통한 숫자판단");
		System.out.println(
				Character.isDigit('A')?"숫자임":"숫자아님");
		System.out.println(
				Character.isDigit(50)?"숫자임":"숫자아님");
		System.out.println(
				Character.isDigit('7')?"숫자임":"숫자아님");
	
		/*
		4] static boolean isLetter(문자)
			: 문자여부를 판단하는 메소드로 특수기호나 숫자형은
			false를 반환한다. 
		 */
		...
		System.out.println(
				Character.isLowerCase('가'));//무조건fasle
	
		/*
		toCharArray() 
			: 문자열을 char형 배열로 반환해주는 메소드. 
			아래는 문자열안에 몇개의 공백문자가 포함되었는지
			확인하는 프로그램이다. 
		 */
