...
public class ExceptionCatch {
	...
	public static void main(String[] args) {
		...
		/*
		예외처리를 할때 필요에 따라 여러개의 catch블록을 사용
		할수있다. 
		-이경우 Exception 객체는 가장 마지막의 블록에서 사용
		해야한다. 
		-Exception객체는 최상위 예외클래스이므로 모든 예외를
		catch하기때문에 첫번째 catch블록에 사용하게되면
		아래부분에서 에러가 발생된다. 
		 */
		...
		try {
			...
			/*
			예외처리에 있어서 디테일한 처리가 필요없다면
			Exception 객체 하나로 catch블록을 구성해도 무방하다.
			단, 각각의 예외클래스로 세분화할수 없다는 단점은
			있다. 
			 */
