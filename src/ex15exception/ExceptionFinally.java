...
/*
	...
	: 예외발생 여부에 상관없이 try문으로 진입했을때 반드시 
	실행해야 하는 코드가 있는경우 기술하는 블럭
	1]try~catch : 예외를 직접 처리할때 사용
	2]try~catch~finally : 예외를 직접 처리한후 반드시 실행할
	문장이 있을때 사용
	3]try~finally : 예외는 외부로 던지고 예외발생과 상관없이
	실행할 문장이 있을경우 사용
	
	※단 try문에서 System.exit(0) 절을 만나게되면 이는 프로그램
	자체종료가 되므로 finally절은 실행되지 않는다. 
 */
public class ExceptionFinally {
	
	/*
	호출되었을때 런타임 오류가 무조건 발생하도록 생성된 메소드로
	예외가 발생될때 직접 처리하지 않고 호출된 지점으로 예외객체를
	던진다. 
	 */
