...
대표적인 하위 클래스이다.
-main메소드에서 무시하기 위해 throws하더라도 오류가 발생
되므로 반드시 예외처리를 해야한다. 
-예외발생시 JVM은 해당 예외클래스를 객체화해서 프로그램쪽
으로 전달하게된다. 
 */
public class ExceptionBasic02 {
	...
	/*
	참조변수로 선언만하고 객체를 생성하지 않은 상태로
	아래 NullPointerException에서 테스트함.
	 */
	static Date toDay;// = new Date();
	/*
	주석처리된 new 부분을 해제하면 예외발생되지 않음.
	 */
	...
	public static void main(String[] args) {
		...
		2-1] ArrayIndexOutOfBoundsException
			: 배열의 크기를 벗어난 인덱스를 사용할 경우 발생되는
			예외
		 */
		int[] intArr = new int[2];//배열의크기는 2
		try {
			intArr[0] = 100;
			...
			intArr[2] = 300; // <- 예외발생지점			
			/*
			위에서 예외가 발생하므로 해당 문장은 실행되지않음.
			 */
			System.out.println("앗~이방은?"+ intArr[2]);
		}
		catch(ArrayIndexOutOfBoundsException e) {
			/*
			try문에서 발생된 예외를 JVM이 객체화하여 catch쪽
			으로 전달하면 해당 예외객체를 받아서 적절하게
			예외처리한다. 
			 */
			System.out.println("예외가 발생했어요");			 
			System.out.println("예외메세지:"+ 
					e.getMessage());
			e.printStackTrace();
			/*
			getMessage() : 예외에 대한 간략한 메세지만 표시
			printStackTrace() : 예외가 발생할때의 에러메세지
				를 그대로 화면에 출력해줌. 개발시 훨씬 더 
				사용빈도가 높음.
			 */
		}
		...
		*/
		...
		try {
			...
			String strAge = 
					sc.nextLine(); //<- "10살"형태로입력
			int ageAfter10 = 
					Integer.parseInt(strAge) + 10; //<- 예외발생
		...
		*/
		...
		2-4]NullPointerException
			: 참조변수가 참조할 객체가 없는 상태에서 멤버를 
			호출할때 발생되는 예외이다. 
			참조변수가 null값을 가지게되면 참조할 객체가 없다는
			뜻이므로 반드시 객체생성후 멤버를 호출해야 한다.
		 */
		System.out.println("toDay="+ toDay);
		try {
			//예외발생 지점을 try밖으로 옮기면 예외발생 즉시 실행이
			//중지된다. 
		...
		/*
		※빈문자열과 null값의 차이
		"" : 빈문자열로 null이 아니다. String객체는 생성되었으나
			값이 없는 상태를 말한다. 
		null : 객체 자체가 생성되지 않는 상태를 말한다. 
		 */
		String emptyString = "";
		System.out.println("emptyString의 문자열길이:"+
				emptyString.length());
		String nullString = null;
		try {
			System.out.println("nullString의 문자열길이:"+
				nullString.length());
		}
		catch(NullPointerException e) {
		...
		try {
			/*
			Object클래스 자체를 다른 타입으로 형변환할수 없다.
			아래 예제를 통해 이해하도록 하자. 
			 */
		...
		/*
		Object클래스와 String클래스간의 형변환을 확인하기
		위한 참조코드
		 */
		String str = "형변환되나요?";
	...
	/*
	매개변수로 전달된 String객체를 Object클래스로 받으므로
	둘 사이에 상속관계가 확인된다.(자동형변환 됨)
	 */
