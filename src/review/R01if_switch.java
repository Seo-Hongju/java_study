...
public class R01if_switch {
	...
	public static void main(String[] args) {
		...
		/*
		주의] 조건은 항상 if문 뒤에 나오게 된다. else문 뒤에는
		조건이 들어가지 않는다. 
		 */
		...
		/*
		if문의 조건을 통해 구간을 정할때는 반드시 논리AND(&&)
		를 사용해야 한다. 
		 */
		
		/*
		다음와 같은 모양을 출력하시오. 단 숫자는 홀수만 
		입력받는걸로 한다. 
		*
		**
		***
		...
		*/	
		...
		for(int i=1 ; i<=FLOOR ; i++) {
			//증가하는 부분을 알기위해 2로나눈값을 무조건 올림
			int half = (int)Math.ceil(FLOOR/2);/*
					ceil()함수는 반환타입이 double이므로
					int형으로 형변환 해야함.
			 	*/
			if(i<=half) {
			...
			System.out.println();/*하나의 층을 출력한후
			 			줄바꿈 처리 */
		}
		...
		if(isTrue==true) {
			System.out.printf("%d월의 날짜수는 %d입니다.",
				month, days);
		}
