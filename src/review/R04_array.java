...
public class R04_array {
	...
	static void randomCreate(int[] lotto) {
		while(true)
		{
			...
			if(isOverlap==false) {
				...
				/*
				반복문에서 continue를 만나게되면 그 이후에 나오는 
				문장들은 실행되지 않고, 반복문의 처음으로 돌아가서
				조건을 검사한후 반복여부를 판단한다. 
				 */
				...
				/*
				반복문안에서 break를 만나면 가장 가까운 반복문 
				하나를 탈출하게된다. 따라서 반복문이 중첩되었다면
				탈출여부를 신중히 판단해야 한다. 
				 */
	...
	public static void main(String[] args) {
		...
		/*
		시나리오]1차원배열을 생성과 동시에 초기화하고 배열의 
		요소에 중복값이 있는지 검사하는 프로그램을 작성하시오.
		중복이 없는경우에는 true, 중복이 있는경우에는 false를 
		반환한다.
		 */
