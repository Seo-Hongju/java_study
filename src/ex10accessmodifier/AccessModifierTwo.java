...
public class AccessModifierTwo {
	...
	public void access() {
		...
		/*
		AccessModiferOne의 멤버중 private으로 선언된 멤버에는
		접근이 불가능함. 서로 다른 외부 클래스이기 때문.
		
		※private으로 선언된 멤버를 외부에서 접근할때는 public
		으로 선언된 메소드를 통해 간접적으로 접근해야 함. 
		 */
		//[호출불가] - private 이므로
		/*System.out.println("one.privateVar="+ 
				one.privateVar);*/
		System.out.println("one.defaultVar="+ 
				one.defaultVar);
		System.out.println("one.publicVar="+ 
				one.publicVar);
		...
		/*
		One클래스와 Two클래스가 동일한 패키지에 선언되었으므로
		객체생성이 가능하다. 
		 */
